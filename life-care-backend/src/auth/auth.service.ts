...
export class AuthService {
  ...
  async validateUser(email: string, password: string): Promise<any> {
    try {
      ...
      // Primeiro tenta verificar com argon2 (novo formato)
      let isPasswordValid = false;
      try {
        isPasswordValid = await argon2.verify(user.password, `${password}${pepper}`);
      } catch (_) {
        // Se o hash antigo não for argon2, o verify pode lançar erro; ignoramos e tentamos bcrypt
        isPasswordValid = false;
      }
      // Fallback: suporta usuários antigos com bcrypt
      if (!isPasswordValid) {
        try {
          isPasswordValid = await bcrypt.compare(`${password}${pepper}`, user.password);
        } catch (_) {
          isPasswordValid = false;
        }
      }
  ...
  async registerPatient(
    ...
    try {
      ...
      const newPatient = await this.usersService.create({
        ...
        // Passa a senha em texto; o UsersService fará o hash (argon2 + pepper)
